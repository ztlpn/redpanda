...
std::ostream& operator<<(std::ostream& o, coordinator::errc e) {
    ...
    case coordinator::errc::other:
        return o << "coordinator::errc::other";
    }
}
...
coordinator::sync_get_last_added_offset(
  model::topic_partition tp, model::revision_id topic_rev) {
    ...
    if (topic_rev < topic.revision) {
        vlog(
          datalake_log.debug,
          "asked offsets for tp {} but rev: {} is obsolete, got rev: {}",
          tp,
          topic_rev,
          topic.revision);
    ...
    if (topic.lifecycle_state != topic_state::lifecycle_state_t::live) {
        vlog(
          datalake_log.debug, "topic {} rev: {} already closed", tp, topic_rev);
        co_return errc::revision_mismatch;
    }
...
coordinator::update_lifecycle_state(
  const model::topic& t, model::term_id term) {
    ...
    case topic_state::lifecycle_state_t::live: {
        if (topic.revision >= topic_table_.last_applied_revision()) {
        ...
        if (topic_md && topic_md->get().get_revision() == topic.revision) {
            // topic still exists
            co_return ss::stop_iteration::yes;
        }
