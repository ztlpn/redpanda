...
update_partition_spec_action::build_updates() && {
    ...
    chunked_hash_map<ss::sstring, partition_field::id_t> field_name2id;
    for (const auto& pspec : table_.partition_specs) {
        ...
        for (const auto& field : pspec.fields) {
            field_name2id[field.name] = field.field_id;
        }
    }
    ...
    auto cur_field_id = last_field_id + 1;
    for (const auto& field : new_spec_.fields) {
        ...
        auto it = field_name2id.find(field.name);
        if (it != field_name2id.end()) {
            resolved_field.field_id = it->second;
        } else {
            resolved_field.field_id = cur_field_id;
            cur_field_id += 1;
        }
